...
//Dummy function because opencl.cpp assumes cleanup() is defined
void cleanup(){
}
...
int main(int argc, char *argv[]) {
  ...
  if (test_to_run < 0 || test_to_run > 7 || device_to_run < 0 || device_to_run >= num_devices)
    incorrectArgs = true;
  ...
  for (d; d < num_devices; d++) {
    ...
    if ( test_to_run == 0 || test_to_run == 5 || test_to_run == 2 || test_to_run == 3 || test_to_run == 4 || test_to_run == 6 || test_to_run == 7) {
      ...
      if (p) {
        ...
        float PercentError = fabs(quartusFreq - MeasuredFreq) / (quartusFreq) * 100;
        if (PercentError  < 2)
