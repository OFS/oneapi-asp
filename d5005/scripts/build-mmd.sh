...
CMAKE_OCL_BSP_AFU_ID_ARG="-DOCL_BSP_AFU_ID=D5005"

export CC=${CC:-$(which gcc)}
export CXX=${CXX:-$(which g++)}
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$BSP_ROOT/build/json-c/install/lib64
#cmake -DCMAKE_BUILD_TYPE="$BUILD_TYPE" -DOPENCL_ASE_SIM="$SET_ASE" -DCMAKE_INSTALL_PREFIX="$BSP_ROOT/linux64" "$CMAKE_OPAE_ARG" "$BSP_ROOT/../common/source" || exit
...
#delete copied files from commom
#for item in "${copy_delete_list[@]}"
#  do
#	echo $item
#     rm -r $BSP_ROOT/source/$item
#  done
