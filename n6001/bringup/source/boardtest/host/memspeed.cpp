...
int memspeed (
  ...
  //For cards with 32GBytes running the kernels with 8GB fails, this is to avoid this.
  if (total_bytes_used > 4294967296)
    total_bytes_used = 4294967296;
  ...
  for ( int k = 0; k < NUM_KERNELS; k++) {
    ...
    for (int b = 0; b < NUM_DIMMS; b++) {
      ...
      if (ddatain) {
        if (CL_SUCCESS == clReleaseMemObject(ddatain)) {
          ddatain = NULL;
        }
      }
    }
