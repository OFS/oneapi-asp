...
size_t ocl_test_all_global_memory( )
{
  ...
  printf("Writing %llu MB to global memory ... \n", max_size / MB);
  ...
  printf("Reading %llu bytes from global memory ... \n", max_size / MB);
  bytes_rem = max_size;
  offset = 0;
  sum_time_ns = 0.0;
  cl_ulong errors = 0;
  cl_ulong compare_count = 0;
  cl_ulong chunk_cnt = 0;
  cl_ulong chunk_errors = 0;
  while (bytes_rem > 0) {
    cl_ulong chunk = bytes_rem;
    if (chunk > host_size) chunk = host_size;
    ...
    for (cl_ulong i = 0; i < chunk / sizeof(cl_ulong); ++i) {
      ...
      if (hostbuf[i] != (i + offset)) {
        ...
        if (chunk_errors <= 32) //only print 32 errors
          printf("Verification failure at element %llu; chunk_cnt %llu;, expected 0x%llx \\ %llu but read back 0x%llx \\ %llu\n", i, chunk_cnt,i, i, hostbuf[i], hostbuf[i]);
      }
    }

    if (chunk_errors>0) {
        printf("chunk_errors for chunk %llu was %llu \\ 0x%llx\n",chunk_cnt,chunk_errors,chunk_errors);
  ...
  printf("total chunk_cnts was %llu \\ 0x%llx\n",chunk_cnt, chunk_cnt);
  //
  // 5. Do Verification
  if (errors == 0)
    printf ("Successfully wrote and readback %llu MB buffer\n", max_size / 1024 / 1024);
  else {
    printf ("Wrote and readback %llu MB buffer\n", max_size/1024/1024);
    printf ("Failed write/readback test with %llu errors out of %llu \\ 0x%llu comparisons\n",errors, compare_count,compare_count);
  }
  printf("\n");
