...
def setup_bsp(bsp_root, env_vars, bsp, verbose, device_family):

    top_level_name, ofs_prefix = get_device_information(device_family)
    ...
    bsp_common_dir = os.path.join(bsp_root, "hardware", "common")
    bsp_common_build_dir = os.path.join(bsp_common_dir, "build")
    ...
    bsp_ip_dir = os.path.join(bsp_common_dir, 'build','ip') #TOREMOVE changed to point to common
    ...
    filelist_path = os.path.join(bsp_common_build_dir, "filelist.txt")
    ...
    ofs_pr_afu_qsf_file=os.path.join(bsp_qsf_dir, '{}pr_afu.qsf'.format(ofs_prefix))
    ...
    orig_qpf_file=os.path.join(bsp_qsf_dir, "".join([top_level_name, '.qpf']))
    ocl_qpf_file=os.path.join(bsp_dir, "".join([top_level_name, '.qpf']))
    ...
    update_qpf_project_for_opencl_flow(ocl_qpf_file, top_level_name)

    # update quartus project files for opencl
    quartus_qpf_file=orig_qpf_file
    #print ("update qpf project for afu %s\n" % quartus_qpf_file)
    update_qpf_project_for_afu(quartus_qpf_file, top_level_name)
    ...
    if(device_family =='d5005'):
        ...
        #replace_lines_in_file(afu_flat_qsf_path, 'set_global_assignment -name SOURCE_TCL_SCRIPT_FILE ../setup/suppress_warning.tcl', '#set_global_assignment -name SOURCE_TCL_SCRIPT_FILE ../setup/suppress_warning.tcl')
        replace_lines_in_file(ofs_pr_afu_source_tcl_file, 'set FIM_SCRIPT_DIR "../setup"', 'set FIM_SCRIPT_DIR "./syn/setup"')
        #hierarchy is different
        replace_lines_in_file(ofs_pr_afu_source_tcl_file, '"../../..', '".')
        # set up paths correctly to find afu_main.tcl
        replace_lines_in_file(ofs_pr_afu_source_tcl_file, "#set_global_assignment -name SOURCE_TCL_SCRIPT_FILE \"../.././ofs-common/src/fpga_family/stratix10/afu_main.tcl\"", "set_global_assignment -name SOURCE_TCL_SCRIPT_FILE $::env(BUILD_ROOT_REL)/ofs-common/src/fpga_family/stratix10/afu_main.tcl")
    ...
    FIM_sdc_file=os.path.join(bsp_qsf_dir, "".join([top_level_name, '.out.sdc']))
...
def update_qpf_project_for_opencl_flow(qpf_path, top_level_name):
    ...
    with open(qpf_path, 'a') as f:
        ...
        f.write('PROJECT_REVISION = "{}"\n'.format(top_level_name))

# update quartus project for afu compile flow
def update_qpf_project_for_afu(qpf_path, top_level_name):
    chmod_plus_w(qpf_path)
...
def main():
    ...
    parser.add_argument("device_family", help="Name of device family to configure")
    ...
    bsp_root = os.environ.get("OFS_OCL_SHIM_ROOT")
    setup_bsp(bsp_root, env_vars, args.board_name, args.verbose, args.device_family)
