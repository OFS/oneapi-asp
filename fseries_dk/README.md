...
## Overview
This repository contains files necessary to generate ASP for the OFS fseries_dk
...
* `fseries_dk_usm`: ASP that supports shared virtual memory between host and device. This 
variant is the same as the non-USM variant with the addition of the USM path between 
the kernel-system and the host.

* `fseries_dk`:  DMA-based ASP that supports local memory and host memory interfaces for the 
kernel system.
...
Example compilation command: <br>
  `$OFS_ASP_ROOT/$ aoc -v -board=fseries_dk
