...
# amount of disk space and see if they are the same in the fseries-dk
# and fseries-dk_usm directories. If so replace the fseries-dk_usm copy with
# a hard link to version of the file in fseries-dk directory
else
  echo "Deduplicating large files in hardware fseries-dk and fseries-dk_usm direcotry"
  ...
  for f in "${dups[@]}"; do
    if [[ -e "$BSP_ROOT/hardware/fseries-dk/$f" && 
          -e "$BSP_ROOT/hardware/fseries-dk_usm/$f" ]];
    then
      ln -f "$BSP_ROOT/hardware/fseries-dk/$f" "$BSP_ROOT/hardware/fseries-dk_usm/$f"
    fi
  done
fi
