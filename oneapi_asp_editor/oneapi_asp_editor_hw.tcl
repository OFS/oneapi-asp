# +-----------------------------------
# | 
# | oneAPI Kernel Wrapper "Display PoC - oneAPI Kernel Wrapper IP" v1.0
# | Kernel System Wrapper IP Display PoC. Constitutes the kernel_system and pipeline bridges to data and control ports
# |
# +----------------------------------- 
...
# 
# kernel_wrapper::helper namespace contains lists of properties and procedures to add IP parameters (TODO: add interface/fileset etc as required)
#
...
proc init_module { prop_list val_list } {
    #TODO: add chk in all procedures for mismatch in lengths of val & prop lists - decide shorter allowed or must be equal - no if shorter then maybe missed a value accidently - must be equal, figure out send error (send_message)
...
# Procedure to generate Verilog Header file
# TOOD: Ideally IP should use HDL_PARAMETERS and this procedure should not be needed
proc generate_vh {} {
    ...
    for {set i 0} {$i < $num_global_mem_systems} {incr i 1} {
      ...
      set global_mem_num_mem_banks [get_parameter_value MEM_0_NUM_BANKS]
...
# Procedure to generate OFS ASP package file 
# TOOD: Ideally IP should use HDL_PARAMETERS and this procedure should not be needed
...
proc synth { oneapi_kernel_wrapper } {
...
proc elaboration {} {
    kernel_wrapper::main::main_elab $::parameter_properties $kernel_wrapper::main::main_dynamic_param
    kernel_wrapper::global_mem::mem_elab $::parameter_properties $kernel_wrapper::global_mem::mem_dynamic_param
    kernel_wrapper::usm::usm_elab $::parameter_properties $kernel_wrapper::usm::usm_dynamic_param
    #kernel_wrapper::dma::dma_elab $::parameter_properties $kernel_wrapper::dma_dynamic_param
    kernel_wrapper::channels::chan_elab $::parameter_properties $kernel_wrapper::channels::chan_dynamic_param
    kernel_wrapper::project::project_elab $::parameter_properties $kernel_wrapper::project::project_dynamic_param
    return
}
...
# TODO: the set_module_property INSTANTIATE_IN_SYSTEM_MODULE should be false - HDL_PARAMETERS set to true needs this to be false I think? - true only if validation callback or composition callback set
...
"Display PoC - Kernel System Wrapper IP for oneAPI FPGA Acceleration Flow. TBD e.g. Constitutes the kernel_system and pipeline bridges to data and control ports." \
...
namespace eval kernel_wrapper::main {
...
namespace eval kernel_wrapper::global_mem {
    namespace export mem_elab
    namespace export mem_dynamic_param
    # TODO: get maximum value from oneAPI compiler team
    set GLOBAL_MEM_MAX 4
    ...
    "" "" true true "" "" \
    true "" "TBD - The number of global memory systems the oneAPI kernel connects to. The global memory system usually consists of the on-board external memory connected to FPGA or Unified Shared Memory (USM). If your board has multiple memory banks of same type & size, these can be grouped as a single global memory system. TODO: Include Block RAM info?"] \
                               [list SNOOP_PORT_ENABLE "Snoop port enable" "Global Memory(On-board)" BOOLEAN false "" "" "" "" "" true true "" "" false "" "TBD"] \
    ...
    "" "Bits" "" "" "" "" \
    "" "" "TBD - should point to additional documentation - Data width for Global Memory $i"] \
    ...
    "" "Words" "" "" "" "" \
    "" "" "TBD - guidance on how user can tune this/decide this value , not exposing interleaved_bytes to user - what guidance to give on this?"] \
...
namespace eval kernel_wrapper::usm {
    ...
    set usm_static_param [list \
      [list ENABLE_USM "Unified Shared Memory Interface" "Unified Shared Memory(USM)" BOOLEAN true "" "" "" \
      "" "" true true "" "" \
...
namespace eval kernel_wrapper::dma {
    namespace export dma_elab
    namespace export dma_dynamic_param
    ...
    # Adding dynamic parameters (controlled by static parameter settings)

    # Elaboration procedure for DMA tab
    proc dma_elab {} {    
      return
    }
}
...
namespace eval kernel_wrapper::channels {
    ...
    for {set i 0} {$i < $IO_CHAN_MAX} {incr i} {
        ...
        lappend chan_dynamic_param \
    [list CH_${i}_DATA_WIDTH "Data Width for I/O Channel $i" "Channel $i" INTEGER 64 "" "" "" \
    "" "" "" "" "" "" \
...
namespace eval kernel_wrapper::project {
